# The Transparency Platform (TTP) Deploy Guide (Free-Only, EU)
...
- Identity: external Bank ID / government ID bridge issuing short-lived verification tokens to TTP.
...
- `PLATFORM_IDENTITY_PROVIDER_MODE=bridge`
- `PLATFORM_IDENTITY_BRIDGE_SECRET=<shared-secret-with-your-identity-gateway>`
...
- `PLATFORM_IDENTITY_PROVIDER_MODE=bridge` but `PLATFORM_IDENTITY_BRIDGE_SECRET` is missing.

## 5) Launch Smoke Test
1. `GET /` returns UI.
2. Verify identity via your Bank ID / government ID gateway, then register and log in.
