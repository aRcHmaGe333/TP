...
def test_validate_runtime_config_requires_identity_bridge_secret(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("PLATFORM_ENV", "prod")
    monkeypatch.setenv("PLATFORM_SECRET", "non-default")
    monkeypatch.setenv("PLATFORM_DATABASE_URL", "postgresql+psycopg://u:p@localhost/db")
    monkeypatch.setenv("PLATFORM_CORS_ORIGINS", "https://ttp.example")
    monkeypatch.setenv("PLATFORM_IDENTITY_PROVIDER_MODE", "bridge")
    monkeypatch.delenv("PLATFORM_IDENTITY_BRIDGE_SECRET", raising=False)
