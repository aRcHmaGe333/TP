...
def get_identity_token_max_age() -> int:
    return _get_int("PLATFORM_IDENTITY_TOKEN_MAX_AGE", 900)
...
def get_identity_provider_mode() -> str:
    return os.getenv("PLATFORM_IDENTITY_PROVIDER_MODE", "bridge").strip().lower()


def get_identity_bridge_secret() -> str:
    return os.getenv("PLATFORM_IDENTITY_BRIDGE_SECRET", "").strip()
...
def validate_runtime_config() -> None:
    ...
    if get_identity_provider_mode() == "bridge" and not get_identity_bridge_secret():
        raise RuntimeError(
            "PLATFORM_IDENTITY_BRIDGE_SECRET must be set when PLATFORM_IDENTITY_PROVIDER_MODE=bridge"
        )
