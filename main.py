...
try:
    from . import auth, config, identity
    ...
    from .schemas import (
        ...
        CommentCreate,
        CommentResponse,
        DevIdentityVerificationRequest,
...
app = FastAPI(title="The Transparency Platform (TTP)", version="0.3.0")
...
def to_user_response(user: User) -> UserResponse:
    return UserResponse(
        ...
        identity_country_code=user.identity_country_code,
...
def require_identity_claims(token: str) -> identity.IdentityClaims:
    claims = None
    bridge_secret = config.get_identity_bridge_secret()
    if bridge_secret:
        claims = identity.load_verification_token(token, secret_key=bridge_secret)
    elif config.is_dev_env():
        claims = identity.load_verification_token(token)
    if not claims:
        raise HTTPException(status_code=400, detail="A valid verified identity token is required")
    if claims.provider not in {"bankid", "government_id"}:
        raise HTTPException(status_code=400, detail="Unsupported identity provider")
    return claims


def redact_identity_subject(subject_key: str) -> str:
...
@app.post("/api/identity/dev/verify", response_model=IdentityVerificationResponse)
def dev_verify_identity(payload: DevIdentityVerificationRequest) -> IdentityVerificationResponse:
    if not config.is_dev_env():
        raise HTTPException(status_code=404, detail="Identity dev verification is only available in dev")

    claims = identity.build_dev_claims(
    ...
    return IdentityVerificationResponse(
        ...
        verification_token=identity.create_verification_token(claims),
    )
...
def register(
    ...
    identity_claims = require_identity_claims(payload.identity_verification_token)
    existing_identity = session.exec(
        select(User).where(User.identity_subject_key == identity_claims.subject_key)
    ).first()
    if existing_identity:
        raise HTTPException(
            status_code=409,
            detail="A verified person can only have one account on The Transparency Platform",
        )
    ...
    user = User(
        ...
        legal_name=identity_claims.legal_name,
        email=payload.email,
        ...
        identity_provider=identity_claims.provider,
        identity_subject_key=identity_claims.subject_key,
        identity_country_code=identity_claims.country_code,
        identity_verified_at=identity_claims.verified_at,
    )
    session.add(user)
    session.commit()
    ...
    record_audit_event(
        ...
        details={
            "display_name": user.display_name,
            "identity_provider": user.identity_provider,
            "identity_subject_ref": redact_identity_subject(user.identity_subject_key or ""),
        },
    )
    session.commit()
...
def login(
    ...
    if not user.identity_verified_at or not user.identity_subject_key:
        raise HTTPException(status_code=403, detail="Verified identity is required for this account")
...
def export_report_csv(
    ...
    return Response(
        ...
        media_type="text/csv",
        headers={"Content-Disposition": "attachment; filename=ttp-export.csv"},
    )
